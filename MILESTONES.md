...
- **Notes / Next action:** Replaced autogenerated placeholder vector stubs with named/owned vector-target constants for all 68000 vectors, including explicit reset/default exception/H-Blank/V-Blank ownership (`$450A`, `$4500`, `$483E`, `$482E`) and null TRAP/reserved vectors. Added per-vector ownership labels for every exception/IRQ entry that currently shares `$4500`, so each vector now has an explicit named owner even before code-body splits. Next action is carving unique handler entry labels from the shared `$4500` body as decode quality improves.
...
- **Notes / Next action:** Trimmed vector symbol export to canonical 64-entry 68000 table and promoted autogenerated `vec_###` names to semantic vector names (`initial_ssp`, `reset_pc`, IRQ/TRAP/reserved labels). Next action is promoting non-vector control-flow targets into subsystem-oriented labels backed by disassembly evidence.
...
### Update 2026-03-09 (vector ownership label split)

- **Summary:** Advanced Milestone 1 ownership granularity by splitting shared `$4500` exception target usage into explicit per-vector labels in the vector template while preserving ROM-accurate addresses.
- **Milestones advanced:**
  - M1: from 10% → 14%
- **Evidence produced:**
  - Files changed: `open/vectors_68k.asm`, `MILESTONES.md`, `PROJECT_PLAN_2W.md`
  - Tests/checks run: `pytest -q` (49 passed); `python scripts/disasm_pass.py --rom "Warsong (USA).md" --out open/disasm --max-insn 40000` (`known_instructions: 19,146`; `unknown_words: 17,072`)
- **Risks / blockers:** Shared `$4500` code body is still not split into distinct verified handler entry points; ownership labels are now in place to make that next split mechanical.
- **Next planned actions (ordered):**
  1. Decode and label the first unique branch point(s) inside the `$4500` shared exception path.
  2. Add first deterministic subsystem-level fixture (battle/map/script) to move M5 beyond decoder infrastructure.
...
### Update 2026-03-09 (iteration 27)

- **Summary:** Started Milestone 3 symbol-map growth by fixing vector-symbol scope and naming quality in bootstrap output.
- **Milestones advanced:**
  - M3: from 0% → 8%
- **Evidence produced:**
  - Files changed: `open-warsong/romkit.py`, `open/symbols.csv`, `tests/test_romkit.py`, `MILESTONES.md`, `PROJECT_PLAN_2W.md`
  - Tests/checks run: `pytest -q` (49 passed); `python scripts/check_rom.py --rom "Warsong (USA).md"`; `python scripts/bootstrap_project.py --rom "Warsong (USA).md" --out open`; `python scripts/disasm_pass.py --rom "Warsong (USA).md" --out open/disasm --max-insn 40000` (`known_instructions`: 19,146 / `unknown_words`: 17,072 / `decoded_instructions`: 36,218)
- **Risks / blockers:** `bootstrap_project.py` still regenerates `open/vectors_68k.asm` with generic placeholders; keep using the hand-owned vector file as canonical until bootstrap vector template is upgraded.
- **Next planned actions (ordered):**
  1. Upgrade `write_vector_asm()` to emit owned/semantic vector aliases compatible with current M1 ownership work.
  2. Promote first non-vector control-flow targets in `open/symbols.csv` to subsystem names with rationale comments.
  3. Add first subsystem fixture (battle/map/script) to start measurable M5 correctness movement.
